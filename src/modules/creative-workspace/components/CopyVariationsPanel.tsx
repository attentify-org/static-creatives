...
export function CopyVariationsPanel({
  ...
  async function handleDownloadSelected() {
    ...
    try {
      ...
      const selectedItems = getExportItems(
        backgrounds,
        result,
        baseLayout,
        canvasWidth,
      ...
      if (selectedItems.length === 1) {
        ...
        if (!node)
          throw new Error("Selected variation is not ready for export");
      ...
      for (const { key } of selectedItems) {
        ...
        if (!node) continue;
        const dataUrl = await toPng(node, PNG_EXPORT_OPTIONS);
        zip.file(`${safeFilename(key)}.png`, dataUrl.split(",")[1], {
          base64: true,
        });
      }
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold">
            Original and generated variations
          </h2>
        </div>
        <button
          ...
          disabled={
            downloadStatus === "loading" || selectedDownloadKeys.size === 0
          }
          ...
          return (
            <div key={background.id} className="flex flex-col gap-5">
              <div className="flex items-center justify-between border-b border-[#eee8f4] pb-3">
                <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-[#756c81]">
                  {background.label}
                </h3>
                <span className="rounded-full bg-[#f2edf8] px-3 py-1 text-xs font-medium text-[#6d28d9]">
                  {background.mode}
                </span>
              </div>

              <div className="flex flex-col gap-3">
                <h4 className="text-xs font-semibold uppercase tracking-[0.16em] text-[#8a8294]">
                  Original creative
                </h4>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  <VariationCard
                    ...
                    onToggleExport={(checked) =>
                      onToggleDownloadSelection(originalKey, checked)
                    }
              ...
              {result ? (
                result.variations.map((group) => (
                  ...
                  >
                    ...
                    {group.reason && (
                      <p className="text-sm text-[#8a8294]">{group.reason}</p>
                    )}
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                      {group.items.map((item) => {
                        ...
                        const isSelected =
                          selectedVariationKey?.backgroundId ===
                            background.id &&
                        ...
                        return (
                          <VariationCard
                            ...
                            subtitle={item.patches
                              .map((patch) => patch.blockId)
                              .join(", ")}
                            exportKey={variationKey}
                            checked={selectedDownloadKeys.has(variationKey)}
                            selected={isSelected}
                            onToggleExport={(checked) =>
                              onToggleDownloadSelection(variationKey, checked)
                            }
                            onSelect={() =>
  ...
  return (
    <div
      className={[
        ...
        selected
          ? "border-[#7c3aed] shadow-[0_12px_34px_rgba(124,58,237,0.16)]"
          : "border-[#e5ddec]",
      ].join(" ")}
    >
      ...
      <button
        ...
        className={[
          ...
          selected
            ...
            : "border border-[#ddd4e8] bg-white text-[#5b21b6] hover:bg-[#f6f0ff]",
        ].join(" ")}
        onClick={onSelect}
      >
